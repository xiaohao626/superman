...
module.exports = {
  ...
  queryPlaceDetailById: (placeId) => {
   ...
   //新增景点套餐
   createPlace: (title,introduce,price,feature,scenicType,classify,address,days) => {
    return new Promise((resolve, reject) => {
      try {
        const placeId = tool.guidNum(global.uniqueCodePrefix.placeId);
        const keys = "placeId,title,introduce,price,scenicType,scenicId,classify,address,turnover,number,feature,grade,days,isDel";
